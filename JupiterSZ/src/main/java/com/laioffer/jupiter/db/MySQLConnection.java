...
public class MySQLConnection {
    ...
    //1.创建一个<ItemType, List<item of this type>>的map
    //2.通过user ID 找到该user收藏的所有item， 放在 stringList里面
    //3.把每一个拿到的 itemID 过一遍sql语句  创建出完整的item信息  组成java object 存到HashMap里（并按照itemType 分类）
    public Map<String, List<Item>> getFavoriteItems(String userId) throws MySQLException {
        ...
        try {
            PreparedStatement statement = conn.prepareStatement(sql);
            for (String itemId : favoriteItemIds) {// 这个for循环 = "private Item getItemByItemID(String itemID){return Item}"
                //并把它存在Map里
                ...
                if (rs.next()) {
                    ...
                    itemMap.get(rs.getString("type")).add(item);//在map里找到对应的type 并把item加进去
                    //For example: itemMap.get(VIDEO).add(item1) => Video对应的 List<Item>.add(item 1)
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    // Get favorite game ids for the given user. The returned map includes three entries like {"Video": ["wowID", "lolID", ...], "Stream": ["csGoID", "lolID", ...], "Clip": ["JustChatId", "5678", ...]}
    //给你ItemIds了,按类别返回所有gameId
